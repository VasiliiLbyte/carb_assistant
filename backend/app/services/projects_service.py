...
async def list_items(session, limit: int = 50, offset: int = 0, filters: dict | None = None):
    return await list_entities(session, Project, limit=limit, offset=offset, filters=filters)


async def get_item(session, item_id: str):
    return await get_entity(session, Project, item_id)


async def create_item(session, payload: dict):
    return await create_entity(session, Project, payload)
